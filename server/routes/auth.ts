...
export default function authRoutes() {
  ...
  router.post('/login', async (req, res) => {
    ...
    if (!configuredEmail || !configuredPassword) {
      return res.status(503).json({ error: 'AUTH_ADMIN_EMAIL dan AUTH_ADMIN_PASSWORD belum dikonfigurasi.' });
    }
    ...
    res.cookie(COOKIE_NAME, signAdminToken(configuredEmail), { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', maxAge: 8 * 60 * 60 * 1000 });
